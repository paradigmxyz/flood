...
def _create_load_test_report_ipynb(
    ...
    from rpc_bench.user_io import notebook_io
    ...
    for test_path in test_paths:
        payload = rpc_bench.load_single_run_test_payload(output_dir=test_path)
        name = payload['name']
    ...
    notebook_io.create_notebook(
        cell_templates=report_template_cells,
        output_path=output_path,
        inputs=inputs,
    )


header_template = """# `rpc_bench` Load Test Report
...
*This report was generated by `rpc_bench`. See the `rpc_bench report --help` command for report generation options. This report can be executed as a Python notebook using the `.ipynb` version of this file.*"""


toc_template = """### Contents
1. [Test Summary](#Test-Summary)
2. [Tests](#Tests)
{toc_lines}"""


def create_header_cell(test_names: typing.Sequence[str]) -> str:
    toc = create_test_toc(test_names=test_names)
    return header_template.format(toc=toc)


def create_test_toc(test_names: typing.Sequence[str]) -> str:
    ...
    toc = toc.rstrip()
    return toc_template.format(toc_lines=toc)


def create_parameters_cell(
...
def create_test_chunks(
    ...
    return [
        ...
        for cell in create_test_chunk(test_name)
    ]
...
def create_test_chunk(
    ...
    return notebook_io.create_cells(
        cell_templates=test_template_cells,
...
report_template_cells: notebook_io.NotebookTemplate = [
    {
        # header
        'type': 'markdown',
        'f': create_header_cell,
        'inputs': ['test_names'],
    },
    {
        # imports
    ...
    },
    {
        ...
        'content': """
            ...
            test_payloads = {{
                test_name: rpc_bench.load_single_run_test_payload(output_dir=test_path)
                for test_name, test_path in test_paths.items()
            }}

            results_payloads = {{
                test_name: rpc_bench.load_single_run_results_payload(output_dir=test_path)
                for test_name, test_path in test_paths.items()
            }}
        """,
        'inputs': [],
    ...
    },
    {
        # Tests chunks
        'type': 'chunk',
        'f': create_test_chunks,
        'inputs': ['test_names'],
    },
]


test_template_cells: notebook_io.NotebookTemplate = [
    {
        # test header
        'type': 'markdown',
    ...
    },
    {
        ...
        'content': """
            ...
            rpc_bench.print_load_test_summary(results_payload['test'])
    ...
    },
    {
        ...
        'content': """
            ...
            rpc_bench.print_metric_tables(results, metrics=metrics, comparison=True)
        """,
        'inputs': [],
    },
    {
        ...
        'content': """
            ...
            rpc_bench.plot_load_test_results(test_name=test_name, outputs=results, latency_yscale_log=True)
        """,
        'inputs': [],
    },
    {
